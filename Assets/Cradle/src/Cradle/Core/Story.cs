...
namespace Cradle
{
    ...
    public abstract class Story: MonoBehaviour
    {
		...
		public string StateAsString()
		{
			switch (State)
			{
				...
				case StoryState.Playing:
					return "Playing";
				case StoryState.Paused:
					return "Paused";
				case StoryState.Exiting:
					return "Exiting";
			}
		...
		public float PassageTime { get { return _timeAccumulated + (CurrentTime() - _timeChangedToPlay); } }
		...
		*/
		...
		public void Pause()
		{
			...
			this.State = StoryState.Paused;
			_timeAccumulated = CurrentTime() - _timeChangedToPlay;
		}
		...
		void Enter(string passageName)
		{
			...
			_timeAccumulated = 0;
			_timeChangedToPlay = CurrentTime();
		...
		void ExecuteCurrentThread()
		{
			...
			while (_currentThread != null && _currentThread.MoveNext())
			{
		...
		public void DoLink(StoryLink link, bool force = false)
		{
			if (!force && this.State != StoryState.Idle)
			{
				throw new InvalidOperationException(
					// Paused
					this.State == StoryState.Paused ?
		...
		IEnumerable<Cue> CuesFind(CueType cueType, string linkName = null, bool reverse = false, bool allowCoroutines = true)
		{
			...
			List<Cue> mainCues = CuesGet(this.CurrentPassage != null ? this.CurrentPassage.Name : "", cueType, linkName, allowCoroutines);
